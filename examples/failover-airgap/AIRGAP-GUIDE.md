...
**`provisionBastion`.** The BIG-IP management interfaces have no public address, so you
need a way in. The default deploys the repository's Linux bastion in the first external
subnet with a public IP, restricted to `restrictedSrcAddressMgmt`. If management is reached
over private connectivity, or through AWS Systems Manager Session Manager port-forwarding,
set it to `false` and nothing in the VPC has a public address.
...
`vipRouteTableIds`, `bigIpExternalInterfaceId01/02`, and the bastion's public IP.

## 5. Verifying - do this in both directions

Reach the BIG-IPs through the bastion (`ssh -J ec2-user@<bastion> admin@<mgmt-ip>` or
SSM port-forwarding), then:
...
# 6. From a client in the VPC (the bastion will do), the VIP must answer before and after
